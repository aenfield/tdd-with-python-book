...
class AuthenticateTest(TestCase):
    ...
    def setUp(self):
        ...
        # start database off with at least one user, so we actually have to do something to return None (which we want to do when we don't have the user)
    ...
    def test_creates_new_user_if_needed_for_valid_assertion(self, mock_post):
        mock_post.return_value.json.return_value = {'status': 'okay', 'email': 'a@b.com'}
        found_user = self.backend.authenticate('an assertion')
